...
const DEFAULT_SKILLS: PluginSkillDef[] = [
  {
    ...
    triggerPhrases: ['create a hook', 'add a PreToolUse hook', 'validate tool use', 'implement prompt-based hooks', 'block dangerous commands'],
    coreTopics: ['prompt-based hooks', 'command hooks', 'hook events', 'security best practices', 'input validation'],
  ...
  },
  {
    ...
    triggerPhrases: ['add MCP server', 'integrate MCP', 'configure mcp.json', 'Model Context Protocol', 'stdio server', 'SSE server', 'HTTP server'],
    coreTopics: ['MCP server configuration', 'server types', 'authentication patterns', 'tool naming', 'performance optimization'],
  ...
  },
  {
    ...
    triggerPhrases: ['plugin structure', 'plugin.json manifest', 'auto-discovery', 'component organization'],
    coreTopics: ['directory structure', 'plugin.json format', 'component organization', 'file naming conventions'],
  ...
  },
  {
    ...
    triggerPhrases: ['plugin settings', 'store plugin configuration', '.local.md files', 'per-project settings'],
    coreTopics: ['YAML frontmatter', 'parsing techniques', 'atomic updates', 'gitignore management'],
  ...
  },
  {
    ...
    triggerPhrases: ['create a slash command', 'add a command', 'command frontmatter', 'define command arguments'],
    coreTopics: ['command structure', 'YAML frontmatter', 'dynamic arguments', 'command organization'],
  ...
  },
  {
    ...
    triggerPhrases: ['create an agent', 'add an agent', 'agent definition', 'AI-assisted agent generation'],
  ...
  },
  {
    ...
    coreTopics: ['skill structure', 'progressive disclosure', 'strong triggers', 'skill organization'],
...
const PHASE_GUIDANCE: Record<PluginDevPhase, { purpose: string; actions: string[] }> = {
  'discovery': {
    ...
    actions: ['Clarify plugin purpose', 'Identify requirements', 'Determine scope', 'List expected features']
  },
  'component-planning': {
    purpose: 'Determine needed skills, commands, agents, hooks, MCP',
    actions: ['Identify required components', 'Select relevant skills', 'Plan agent usage', 'Determine MCP needs']
  },
  'detailed-design': {
    purpose: 'Specify each component and resolve ambiguities',
    actions: ['Define component specifications', 'Resolve ambiguities', 'Create detailed plans', 'Ask clarifying questions']
  },
  'structure-creation': {
    purpose: 'Set up directories and manifest',
    actions: ['Create plugin directory', 'Create plugin.json manifest', 'Set up component directories', 'Create README template']
  },
  'component-implementation': {
    purpose: 'Create each component using AI-assisted agents',
    actions: ['Implement commands', 'Implement agents', 'Implement skills', 'Implement hooks', 'Configure MCP']
  },
  'validation': {
    purpose: 'Run plugin-validator and component-specific checks',
    actions: ['Validate structure', 'Validate manifest', 'Validate components', 'Run validation scripts']
  },
  'testing': {
    purpose: 'Verify plugin works in environment',
    actions: ['Test commands', 'Test agents', 'Test hooks', 'Test MCP integration', 'Integration testing']
  },
  'documentation': {
    purpose: 'Finalize README and prepare for distribution',
    actions: ['Complete README', 'Add usage examples', 'Document configuration', 'Prepare for distribution']
  }
};
...
export class PluginDevManager {
  ...
  constructor(config?: Partial<PluginDevConfig>) {
    this.config = {
      ...
      autoPhase: false,
      defaultSkills: ['hook-dev', 'mcp-integration', 'plugin-structure', 'command-dev', 'agent-dev', 'skill-dev'],
  ...
  // Add question
  addQuestion(sessionId: string, phase: PluginDevPhase, question: string): PluginDevState | undefined {
    const session = this.sessions.get(sessionId);
    if (!session) return undefined;
  ...
  // Answer question
  answerQuestion(sessionId: string, phase: PluginDevPhase, questionId: string, answer: string): PluginDevState | undefined {
    const session = this.sessions.get(sessionId);
    if (!session) return undefined;
  ...
  // Update component
  updateComponent(sessionId: string, componentName: string, updates: Partial<PluginComponentSpec>): PluginDevState | undefined {
    const session = this.sessions.get(sessionId);
    if (!session) return undefined;
  ...
  // Set structure
  setStructure(sessionId: string, name: string, directory: string, manifest?: Record<string, unknown>): PluginDevState | undefined {
    const session = this.sessions.get(sessionId);
    if (!session) return undefined;
  ...
  setValidation(sessionId: string, passed: boolean, errors: string[], warnings: string[]): PluginDevState | undefined {
    const session = this.sessions.get(sessionId);
    if (!session) return undefined;
  ...
  completeSession(sessionId: string): PluginDevState | undefined {
    ...
    if (prevCount > 0) {
      ...
      this.stats.averageTimeMinutes = (prevAvg * prevCount + minutesDiff) / this.stats.sessionsCompleted;
    } else {
