...
export interface TrafficControlEvent {
  /** Event type */
  type: "rate_limit" | "throttle_start" | "throttle_end" | "pause" | "resume" | "warning" | "recovery";
  /** Timestamp */
  timestamp: number;
  /** State before event */
...
export class TrafficControlManager {
  ...
  private consecutiveSuccessfulCalls: number = 0;
  ...
  handleRateLimitError(error: unknown, endpoint?: string): TrafficControlState {
    ...
    if (this.stats.consecutiveErrors >= this.config.maxConsecutiveErrors) {
      this.setState("paused", `Max consecutive errors (${this.config.maxConsecutiveErrors}) reached`);
      return "paused";
    }
  ...
  handleSuccess(endpoint?: string): void {
    ...
    if (this.state === "throttling") {
      ...
      if (this.consecutiveSuccessfulCalls >= this.config.autoRecoveryThreshold) {
        this.setState("normal", `Auto-recovery after ${this.config.autoRecoveryThreshold} successes`);
  ...
  getRecentEvents(limit: number = 10): TrafficControlEvent[] {
...
// Singleton getter
export function getTrafficControlManager(config?: Partial<TrafficControlConfig>): TrafficControlManager {
