...
describe("context-analysis helpers", () => {
	it("sorts context task insights by confidence and keeps top actionable files", () => {
		const analyze = vi.fn((taskDescription: string) => {
			if (taskDescription.includes("capability")) {
				return {
					taskDescription,
					suggestedFiles: [
						{ path: "src/tools/index.ts", relevance: 0.91, reason: "", symbols: [], category: "primary" },
						{ path: "src/tools/new-tool.ts", relevance: 0.84, reason: "", symbols: [], category: "secondary" },
			...
			return {
				taskDescription,
				suggestedFiles: [
					{ path: "src/self-improvement-engine.ts", relevance: 0.7, reason: "", symbols: [], category: "primary" },
				],
				relevantSymbols: [],
				confidence: 0.55,
				reasoning: "",
		...
		const candidates: ContextTaskCandidate[] = [
			{ taskDescription: "Improve reliability of a failing evolution tool by updating implementation and tests", minimumConfidence: 0.45, maxFiles: 2 },
			{ taskDescription: "Add a new self-evolution capability tool with tests and tool registration", minimumConfidence: 0.45, maxFiles: 2 },
		];
