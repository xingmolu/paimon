...
- For a web-based browser interface, use \`browserUI({action: 'start', port: 8080})\` to start the browser UI server, \`browserUI({action: 'stop'})\` to stop the server, \`browserUI({action: 'status'})\` to check server status, \`browserUI({action: 'sessions'})\` to list all browser sessions, \`browserUI({action: 'session', sessionId: '...'})\` to get specific session details, \`browserUI({action: 'config'})\` to view configuration, \`browserUI({action: 'stats'})\` to view statistics. The browser UI provides a web-based interface for running evolution sessions - start the server and open the URL in your browser to interact with Paimon through a GUI. Inspired by Aider's --browser feature (Aider browser-ui pattern)
...
- For model settings and metadata, use \`modelSettings({action: 'show'})\` to display all active model settings (main, editor, weak), \`modelSettings({action: 'main'})\` for main model settings, \`modelSettings({action: 'compare', modelId: 'gpt-4o', modelId2: 'claude-3-7-sonnet'})\` to compare two models side-by-side, \`modelSettings({action: 'list'})\` to list all available models, \`modelSettings({action: 'set-main', modelId: 'claude-3-7-sonnet'})\` to set the main model. Shows model metadata including context window, pricing, capabilities (vision, reasoning, streaming, cache). Inspired by Aider's /settings command (Aider Pattern)
- For context identification, use \`context({action: 'analyze', taskDescription: 'Add new tool for file watching'})\` to automatically identify which files need to be edited for a task, \`context({action: 'suggest', filePath: 'src/agent.ts'})\` to find files related to a specific file, \`context({action: 'related', filePath: 'src/tools/file-tools.ts'})\` to get files that share symbols, \`context({action: 'symbols', filePath: 'src/capability-gap.ts'})\` to extract symbols from a file. Analyzes task descriptions, codebase structure, and symbol relationships to suggest relevant files before implementation. Helps reduce context gathering overhead and ensures the right files are identified for changes. (Aider /context pattern)
- For proactive self-improvement suggestions, use \`selfImprovement({action: 'scan'})\` to scan the codebase for improvement opportunities, \`selfImprovement({action: 'suggestions', category: 'security'})\` to filter suggestions by category (code-quality, performance, architecture, capability, reliability, documentation, testing, security), \`selfImprovement({action: 'accept', id: '...'})\` to accept a suggestion, \`selfImprovement({action: 'dismiss', id: '...'})\` to dismiss a suggestion, \`selfImprovement({action: 'stats'})\` to view statistics. Proactively identifies code quality issues, performance improvements, missing capabilities, and competitor patterns. Enables continuous self-improvement through proactive analysis.
...
- Monitor evolution optimization health with \`optimizationDashboard({action: 'report'})\` for a unified dashboard, \`optimizationDashboard({action: 'health'})\` for component scores, \`optimizationDashboard({action: 'bottlenecks'})\` to identify slowdown sources, \`optimizationDashboard({action: 'recommendations'})\` for next optimization opportunities, and \`optimizationDashboard({action: 'compare', successRate: 95, avgTime: 600000, errorCount: 1, capabilitiesUsed: 12})\` to compare the current session against historical averages. Helps prioritize high-leverage self-improvements.
