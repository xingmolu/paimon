...
### Components Created:
${Object.entries(stats.componentsCreated)
	.map(([t, c]) => `- ${t}: ${c}`)
	.join("\n")}
...
### Agents Used:
${Object.entries(stats.agentsUsed)
	.map(([a, c]) => `- ${a}: ${c}`)
	.join("\n")}
`;
}
...
export const pluginDevTool: AgentTool = {
	...
	description:
		"Manage the Plugin Development Toolkit with 8-phase workflow for creating plugins (Claude Code Pattern)",
	parameters: Type.Object({
		action: Type.String({
			description:
				"Action: start, phase, progress, status, sessions, skills, skill, agents, agent, guidance, stats, config, reset, clear, help",
		}),
		description: Type.Optional(
			Type.String({ description: "Plugin description (for start action)" }),
		),
		...
		pluginDirectory: Type.Optional(
			Type.String({ description: "Plugin directory for structure action" }),
		),
		...
		filter: Type.Optional(
			Type.String({ description: "Filter for sessions: active, complete, all" }),
		),
		keepCount: Type.Optional(
			Type.Number({ description: "Number of sessions to keep when clearing" }),
		),
