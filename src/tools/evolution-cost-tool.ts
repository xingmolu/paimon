...
import { getEvolutionCostPredictor } from "../evolution-cost.js";
import type {
	ComplexityLevel,
	CostFactors,
	CostPrediction,
...
evolutionCost({action: 'stats'})`,
	parameters: Type.Object({
		action: Type.String({
			description:
				"Action: predict, quick-check, record, history, factors, risks, similar, stats, accuracy, config, update-config, clear, enable, disable, weights, estimates, help",
		}),
		...
		actualTimeMinutes: Type.Optional(
			Type.Number({
	...
	try {
		switch (action) {
			case "predict": {
				...
				const taskType = String(args.taskType || "capability") as
					| "capability"
					| "reliability"
					| "feature";
			...
			case "accuracy": {
				...
				const lines = recentAccuracy.map(
					(acc: number, i: number) => `Prediction ${i + 1}: ${acc.toFixed(1)}%`,
				);
			...
			case "update-config": {
				...
				if (args.confidenceThreshold !== undefined)
					updates.confidenceThreshold = Number(args.confidenceThreshold);
			...
			case "estimates": {
				...
				const lines = Object.entries(entries).map(
					([k, v]) => `${k}: min ${v.min}m, max ${v.max}m, avg ${v.avg}m`,
				);
...
function formatPrediction(prediction: CostPrediction): string {
	...
	sections.push(
		`Estimated Time: ${prediction.estimatedTimeMinutes} minutes (range: ${prediction.estimatedTimeRange.min}-${prediction.estimatedTimeRange.max}m)`,
	);
