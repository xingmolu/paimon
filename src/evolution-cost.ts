...
export class EvolutionCostPredictor {
   ...
   */
  analyzeTaskFactors(taskDescription: string, taskType: 'capability' | 'reliability' | 'feature'): CostFactors {
   ...
   */
  private findSimilarTasksCount(taskDescription: string, taskType: 'capability' | 'reliability' | 'feature'): number {
  ...
  private extractKeywords(description: string): string[] {
    const stopWords = ['the', 'a', 'an', 'for', 'with', 'and', 'or', 'to', 'from', 'in', 'on', 'of', 'is', 'are'];
   ...
   */
  estimateTime(complexityLevel: ComplexityLevel, factors: CostFactors): { min: number; max: number; avg: number } {
    ...
    if (similarTasks.length > 0) {
      const avgActual = similarTasks.reduce((sum, t) => sum + t.actualTimeMinutes, 0) / similarTasks.length;
   ...
   */
  findSimilarTasks(taskDescription: string, taskType: 'capability' | 'reliability' | 'feature'): HistoricalTask[] {
    const keywords = this.extractKeywords(taskDescription);
  ...
  generateRecommendations(prediction: CostPrediction): string[] {
    ...
    if (prediction.similarTasks.length > 0) {
      recommendations.push(`Reference ${prediction.similarTasks.length} similar past tasks for patterns`);
      
      const successful = this.history.filter(t => 
        prediction.similarTasks.includes(t.taskDescription) && t.success
      );
  ...
  recordOutcome(
    ...
    if (prediction) {
      const accuracy = Math.max(0, 100 - Math.abs(prediction.estimatedTimeMinutes - actualTimeMinutes) * 2);
      ...
      this.stats.averageAccuracy = 
        this.stats.accuracyHistory.reduce((sum, a) => sum + a, 0) / this.stats.accuracyHistory.length;
    }
